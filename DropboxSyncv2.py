#!/usr/bin/env python
"""
A simple Script to synchronize a folder in raspberry pi with a Dropbox application.

Setup: 
* Create a Dropbox Application and get the token. 
* Save the token to a text file ".dropbox_token"
* Put the token file and this script in the root of the raspberry pi folder you
  want to synchronize.
* Execute the script from that folder, or edit the variable SYNC_DIR in this 
  script.
...
be sure you are using version 8 at least).
...
class DropboxSync:
    ...
    def sync_local(self):
        ...
        old_list = dropbox_sync.local_files.keys()
        ...
        old_list = dropbox_sync.local_dirs.keys()
    ...
    def create_remote_folder(self, path):
        ...
        """
        print '\tCreating folder in server:', path
        checkpath = path.replace('\\', '/')
    ...
    def delete_remote_folder(self, path):
        ...
        """
        print '\tFolder deleted locally. Deleting on Dropbox:', path
        path = path.replace('\\','/')
        try:
            self.dropbox_client.files_delete(os.path.join('/',path))
        except:
